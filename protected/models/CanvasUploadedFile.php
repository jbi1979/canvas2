 ...
 * @since 1.0
 */
class CanvasUploadedFile extends CComponent
{
	...
	protected static function collectFilesRecursive($key, $names, $tmp_names, $types, $sizes, $errors)
	{
		if(is_array($names))
		{
			...
			self::$_files[$key] = new CanvasUploadedFile($names, $tmp_names, $types, $sizes, $errors);
	}
	...
	{
		$file = $this->getFilenameUnique($file);
	...
	// 중복된 파일이 있을 경우, 중복되지 않게 파일명 업데이트
	public function getFilenameUnique($filename)
	{
		...
		if($filename) 
		{
			$count = strrpos($fileinfo['name'],'.');
			$expend = substr($fileinfo['name'], $count + 1, strlen($fileinfo['name'])-$count);
			$file = substr($fileinfo['name'], 0, $count);

			$i=1;
			while (file_exists($fileinfo['path'].'/'.$fileinfo['name'])) 
			{
				$fileinfo['name'] = $file.'_'.$i.'.'.$expend;
				$i++;
			}
		}
	...
	/**
	 * @return array the file name and the file path for {@link name}.
	 */
	public function getFileinfo($filename)
	{
		...
		{
			$fileinfo['path'] = (string)substr($filename, 0, $pos);
			$fileinfo['name'] = (string)substr($filename,$pos+1);
		}
